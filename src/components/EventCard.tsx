...
export default function EventCard({ eventTitle, date }: EventProps) {
  ...
  return (
    <div className="flex flex-col items-center space-y-2 w-42 shrink-0 m-4 p-4 bg-[#fcfce6] rounded-xl">
      ...
      <button onClick={() => router.push(`/admin-event-details`)} className="bg-[#448EBF] bg-opacity-20 m-1 p-2">
        Learn More
      </button>
      {/* will need to get rid of event-details hardcode navigation push and instead use the id of the event to navigate to the correct event details */}
